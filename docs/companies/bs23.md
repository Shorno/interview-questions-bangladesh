...
Use a **stack (LIFO — last in, first out)** when the most recent item must be handled first or undone in reverse order.

**Typical stack situations (MCQ)**

| Situation | Why stack |
|-----------|-----------|
| Browser **back** / forward history | Last visited page popped first |
| **Undo** in editor or drawing app | Reverse chronological actions |
| **Matching parentheses** / syntax checking | Open symbols pushed, closed symbols pop |
| **DFS** iterative traversal | Explicit stack instead of recursion |
| **Expression evaluation** (postfix/prefix) | Operands and operators |

**Not a stack**

- Printer job queue, ticket line, BFS layers → **queue (FIFO)**
- “Next highest priority” task → **priority queue / heap**

Pick the option that needs **LIFO** behavior, not FIFO or priority ordering.
